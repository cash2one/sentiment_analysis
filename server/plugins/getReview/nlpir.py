...
libFile = os.path.join( os.path.dirname(__file__), '../../utils/nlpir/libNLPIR.so')
stopwordDict = os.path.join( os.path.dirname(__file__), 'stopword.dict') 
userDict = os.path.join( os.path.dirname(__file__), 'user.dict')
...
def __init__():
    for line in open(userDict,'r').readlines():
        AddUserWord(line.strip('\n'))
    for line in open(stopwordDict,'r').readlines():
